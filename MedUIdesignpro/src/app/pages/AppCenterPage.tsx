...
import {
  ...
  Gauge, BookOpen, CheckCircle, XCircle, Globe,
} from 'lucide-react';
import {
  mockApps, SimSystem, SYS_CATEGORIES, HW_LABELS, HwSupport,
} from '../data/appsData';
...
export function AppCenterPage() {
  ...
  }, [search, category, hwFilter]);

  const stats = useMemo(() => ({
    total: mockApps.length,
    active: mockApps.filter(a => a.status === '0').length,
    totalViews: mockApps.reduce((s, a) => s + a.view_count, 0),
    totalExp: mockApps.reduce((s, a) => s + a.exp_count, 0),
  }), []);
