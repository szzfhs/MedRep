        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            ...
            {filtered[0] && activeCategory === '全部' && !search && (
              <Link to={`/news/${filtered[0].newsId}`} className="group block mb-6">
                <motion.div
                ...
                >
                  <div className="sm:flex">
                    <div className="relative sm:w-64 h-48 sm:h-auto flex-shrink-0 overflow-hidden">
                      <img
                        src={filtered[0].coverImage || '/placeholder.svg'}
                    ...
                    <div className="p-6 flex flex-col justify-between">
                      ...
                      <div className="flex items-center justify-between mt-4">
                        <div className="flex items-center gap-4 text-xs text-[#94A3B8]">
                          <span className="flex items-center gap-1"><Calendar size={12} />{filtered[0].publishTime?.slice(0, 10) ?? '-'}</span>
                          <span className="flex items-center gap-1"><Eye size={12} />{filtered[0].viewCount ?? 0}</span>
                        </div>
                ...
                {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="bg-white rounded-2xl border border-[#E2E8F0] p-5 animate-pulse">
                    <div className="flex gap-4">
                      <div className="w-20 h-16 bg-[#E2E8F0] rounded-xl flex-shrink-0" />
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="text-[#94A3B8] text-xs">{item.publishTime?.slice(0, 10) ?? '-'}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-3 mt-2 text-xs text-[#94A3B8]">
                          <span className="flex items-center gap-1"><Eye size={11} />{item.viewCount ?? 0} 阅读</span>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-[#E2E8F0] p-5">
              ...
              <div className="space-y-2">
                {FIXED_CATEGORIES.slice(1).map((cat) => (
                  <button
                    key={cat}
                    onClick={() => setActiveCategory(cat)}
            ...
            <div className="bg-white rounded-2xl border border-[#E2E8F0] p-5">
              ...
              <div className="space-y-3">
                {newsList.slice(0, 4).map((item) => (
                  <Link key={item.newsId} to={`/news/${item.newsId}`} className="group flex items-start gap-2.5">
